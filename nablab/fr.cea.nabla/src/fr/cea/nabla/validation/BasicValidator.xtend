...
class BasicValidator  extends AbstractNablaValidator
{
	...
	def checkUnusedFunctions(Function it)
	{
		...
		if (!referenced)
			warning('Unused function', NablaPackage.Literals::CALLABLE__NAME)
	}	
			...
			else
			{
				for (i : 0..<spaceIterators.length)
				{
					...
					switch si
					{
						...
						SpaceIteratorRef:
						{
							val actualT = si.getIterator.range.connectivity.returnType.type
...
//							}
	...
	def checkArgs(SpaceIteratorRange it)
	{
		...
		else
		{
			for (i : 0..<args.length)
			{
				val actualT = args.get(i).getIterator.range.connectivity.returnType.type
