...
class Foobar {
	def static void main(String[] args) {
		val Injector injector = new NablaStandaloneSetupGenerated().createInjectorAndDoEMFRegistration();
		
		val rs = new ResourceSetImpl()
		
		val XtextResource res = rs.createResource(URI.createURI("inmemory:/foobar.nabla")) as XtextResource
		
		
		val initialString = '''module Glace2d;
		...
		val RootNode root = res.parseResult.rootNode as RootNode
		
		val out = new ByteArrayOutputStream()
		val dos = new DataOutputStream(out)
		
		val serializer = new SerializableNodeModel(res)
		val ctx = new SerializationConversionContext(res)
		serializer.writeObjectData(dos,ctx)
		
//		val str = new String(out.toByteArray)
		
		
		val XtextResource res2 = rs.createResource(URI.createURI("inmemory:/foobar2.nabla")) as XtextResource
		val in = new ByteArrayInputStream(out.toByteArray);
		val dis = new DataInputStream(in);
		val ctx2 = new DeserializationConversionContext(res2,"foobar")
		
		val serializer2 = new SerializableNodeModel(res2)
