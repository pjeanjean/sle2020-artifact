...
//	}
	...
	def List<? extends SymbolInformation> getSymbols(
		...
		val resource = rs.getResource(resourceDescription.URI,true)
		for (description : resourceDescription.exportedObjects) {
			operationCanceledManager.checkCanceled(cancelIndicator)
			if (description.filter(query)) {
				description.createSymbol(resource) [ symbol |
					symbols += symbol
				]
			}
		}
...
//		println(resource)
//		println(resource.resourceSet)
//		println(description)
		val object = resource.resourceSet.getEObject(description.EObjectURI, true)
		if (object !== null) {
			val location = object.symbolLocation
			if (location !== null) {
