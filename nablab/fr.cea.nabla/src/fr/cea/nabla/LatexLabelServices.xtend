...
class LatexLabelServices 
{
	...
	static def dispatch String getLatex(TimeLoopJob it) { '\\texttt{' + name.pu + '} : \\forall {' + getIterator.latex + '}, \\ '+ body.latex }
	...
	static def dispatch String getLatex(Loop it) { '\\forall {' + getIterator.latex + '}, \\ ' + body.latex }
	...
	{ 
		if (next) getIterator.name.pu + '+1'
		else if (prev) getIterator.name.pu + '-1'
		else getIterator.name.pu
	}
	...
	static def dispatch String getLatex(ReductionCall it) { reduction.name.pu + '_{' + getIterator.latex + '}' + '(' + arg.latex + ')' }
	...
	{ 
		if (init) getIterator.name.pu + "=" + value
		else if (next) getIterator.name.pu + "+" + value
		else getIterator.name.pu
	}
