...
import fr.inria.diverse.ide.LogoIdeSetup;
...
//	LanguageServerImpl server = new LogoIdeSetup().createInjectorAndDoEMFRegistration().getInstance(LanguageServerImpl.class);
	...
	public LanguageServerImpl getOrCreateServer() {
		if(server == null) {
			server = new LogoLSPModule().createInjectorAndDoEMFRegistration().getInstance(LanguageServerImpl.class);
		}
		return server;
	}
